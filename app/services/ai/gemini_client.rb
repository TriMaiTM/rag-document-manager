...
module Ai
  class GeminiClient
    ...
    def embed_documents(inputs:)
      ...
      uri = endpoint_uri
      request = build_request(uri, inputs)
    ...
    def build_request(uri, inputs)
      Net::HTTP::Post.new(uri).tap do |request|
        request["Content-Type"] = "application/json"
        request["x-goog-api-key"] = config.api_key
        request.body = JSON.generate(request_body(inputs))
      end
    end
    ...
    def request_with_retries(uri, request)
      ...
      loop do
        ...
        return response if attempt >= config.max_retries
